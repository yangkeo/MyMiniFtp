...
static unsigned int s_children; //客户端统计连接数
...
int main(int argc, char* argv[])
{
	if (getuid() != 0)
	{
		printf("miniftp must be satrted as root\n");
		exit(EXIT_FAILURE);
	}
	parseconf_load_file("miniftp.conf");
	//ParseConf_Test();
	...
	sess.upload_max_rate = tunable_upload_max_rate;  //获取上传限速
	sess.download_max_rate = tunable_download_max_rate; //获取下载限速
	...
	while(1)
	{
		...
		if(pid == 0)
		{
			...
			close(listenfd);
			//会话
			sess.ctrl_fd = sockConn; //控制套接字在此介入
